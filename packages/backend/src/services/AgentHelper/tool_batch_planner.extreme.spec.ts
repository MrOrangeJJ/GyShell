...
runCase('a later terminal lifecycle call preserves ordinal zero and every call id', () => {
  const planned = planToolCallBatch(
    [
      call('first-read', 'read_file'),
      call('create-tab', 'create_terminal_tab', { connectionId: 'local' }),
      call('last-read', 'read_terminal_tab')
    ],
    createEnvironment()
  )
  ...
  assert.deepEqual(
    ...
    [
      ['first-read', 'execute'],
      ['create-tab', 'not_executed'],
      ['last-read', 'not_executed']
    ]
  )
  assert.equal(planned[1]._gyshellExecution.retryable, true)
})

runCase('a leading close terminal call defers its entire preserved tail', () => {
  const planned = planToolCallBatch(
    [
      call('close-tab', 'close_terminal_tab', { tabIdOrName: 'shell' }),
      call('exec', 'exec_command', { tabIdOrName: 'shell', command: 'after' }),
      call('read', 'read_terminal_tab', { tabIdOrName: 'shell' })
    ],
    createEnvironment({ terminalByReference: { shell: 'terminal-1' } })
  )
...
runCase('a disabled lifecycle call does not serialize independent reads', () => {
  const planned = planToolCallBatch(
    [
      call('first-read', 'read_file'),
      call('create-tab', 'create_terminal_tab', { connectionId: 'local' }),
    ...
    createEnvironment({ disabled: ['create_terminal_tab'] })
  )

  assert.deepEqual(
    planned.map((item) => item._gyshellExecution.mode),
    ['execute', 'not_executed', 'execute']
  )
})
