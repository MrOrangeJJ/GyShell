...
await runCase('a file mutation adapter that ignores stop cannot publish a late result event', async () => {
  ...
  const controller = new AbortController()
  const invocation = (agent as any).createFileToolsNode().invoke(
    {
      sessionId: 'late-write-file',
