...
export class TerminalService {
  ...
  async runCommandNoWait(terminalId: string, command: string): Promise<void> {
    await this.executeCommandInternal(terminalId, command, 'nowait', {
      terminalId,
      onFinished: () => {
  ...
  async runCommandAndWait(
    ...
    return new Promise<CommandResult>((resolve, reject) => {
      ...
      observer.timeoutTimer = setTimeout(() => {
        ...
        resolve({ stdoutDelta: 'Command timed out (120s).', exitCode: -1 })
      }, 120_000)
      ...
      if (opts?.signal) {
        const onAbort = () => {
          ...
          resolve({ stdoutDelta: 'Command aborted by user.', exitCode: -2 })
        }
  ...
  private async executeCommandInternal(
    ...
    if (this.activeTaskByTerminal.has(terminalId)) {
      throw new Error(
        'There is a running exec_command in the terminal tab. Please read the tab to confirm the situation. If you need to end the previous command, use send_char to end it, otherwise wait until it finishes.'
      )
    }
  ...
  private finishActiveTask(terminalId: string, exitCode?: number): void {
    ...
    if (observer) {
      this.cleanupWaiter(taskId)
      observer.onFinished({ stdoutDelta: task.output, exitCode })
    }
  }
