...
- **模型级请求参数**：可添加经过校验的 OpenAI 兼容请求体覆盖字段，同时保护运行时管理的字段。
- **可信的命令输出语义**：明确区分完整、部分捕获与展示截断，并可继续读取已保留的长输出。
- **更清爽的 Seamless 活动展示**：通过紧凑的多层展开结构查看工具活动，并准确呈现警告与失败状态。
- **Agent Setting 自动保存**：Profile 激活期间的更改会自动写回，无需手动覆盖。
- **工具与 Skills 更清晰**：工具说明更简短，Skills 只从 GyShell 管理目录和 `~/.agents/skills` 自动发现。
- **稳定性修复**：流式多工具调用保持完整，终端与 SSH 恢复更稳健，后台 SSH 标签仍可监控，Windows 终端缩放保持同步，聊天面板也会保留当前选中的会话。
...
- 长会话智能压缩、独立压缩模型链路、可见的 `[CTX COMPACTED]` 压缩边界标记、高效的退出时 checkpoint 序列化，以及历史仍超过紧急上下文预算时可从持久化阅读进度继续的恢复机制。
...
- 对话面板支持 `传统模式` 与 `无感模式` 两种 Agent 活动呈现方式；无感模式通过紧凑的多层结构呈现工具细节，并准确标记警告与失败级别。
- 支持通过 `memory.md` 注入持久记忆；当应用 Agent Setting 配置档时，memory 会按活跃配置档隔离。
- 支持多模态输入链路（文字 + 图片）。
- 支持 OpenAI 兼容接口模型，可按模型添加经过校验的文本、数字、布尔值和 JSON 请求体覆盖字段，同时保护运行时管理的字段。
- 执行规划前会依据原始响应标识校准流式多工具调用，确保调用 ID、序号、参数和结果在执行与恢复过程中保持完整，并能自动回退处理畸形的空工具调用结束流。
- 命令工具会明确提供执行、捕获和展示状态，让 Agent 能区分完整输出、不完整捕获与受限的展示节选，并继续读取已保留的结果。
...
- 本地与 SSH 终端会隐藏内部 shell 初始化和 Unix 调度命令，并通过更严格的启动重试、与 prompt 绑定的输入门控和输出流控保持状态一致。
...
- 即使后台 SSH 标签没有挂载到 terminal panel，也仍可出现在已有或之后创建的 monitor panel 中。
